...
@pytest.fixture
def container():
    container = Container(config={'dcinternal':{'scope':'data-prd-keyvault', 'key_name':'dcinternal-account-key'}})
...
def test_module(container):
    ...
    result = container.blob_container_service().select(dbutils=dbutils, container_name='', options='', file_path='')
