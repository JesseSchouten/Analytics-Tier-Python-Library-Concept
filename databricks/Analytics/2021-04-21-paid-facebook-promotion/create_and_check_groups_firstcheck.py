...
query = """
SELECT axinom_id, subscription_created_date, platform, group FROM SAMPLES_20210421
"""
...
df[['axinom_id','group']].groupby('group').nunique()
...
tracking
| where videoPlayTime >=0 and isTest == false and startofday(timestamp) >= startofday(datetime('2021-4-21')) and startofday(timestamp) <= startofday(datetime(now)) - 1d and channel == '{}'
...
def add_column_avg_viewing_time_past_x_weeks(viewing_df, current_date = datetime(2021,4,20,0,0,0), past_days = 56):
...
selected_viewing_df = selected_viewing_df.merge(df[['axinom_id','group','platform']],how='left',left_on=['user_id'],right_on=['axinom_id'])
current_date = datetime(2021,5,4,0,0,0)
print("Control group:")
display(add_column_avg_viewing_time_past_x_weeks(selected_viewing_df[selected_viewing_df['group']=='control_group'],current_date,past_days=14))
print("Experimental group:")
display(add_column_avg_viewing_time_past_x_weeks(selected_viewing_df[selected_viewing_df['group']=='experimental_group'],current_date,past_days=14))
...
viewing_df_grouped =selected_viewing_df[['user_id','group','video_playtime_minutes']].groupby(['user_id','group']).sum().reset_index()
nr_dates = len(selected_viewing_df['timestamp'].unique().tolist())
viewing_df_grouped['video_playtime_minutes'] = (viewing_df_grouped['video_playtime_minutes'] / nr_dates) * 7
nr_zero_users = len(viewing_df_grouped[viewing_df_grouped['video_playtime_minutes'] == 0]['user_id'].unique())
nr_total_users = len(viewing_df_grouped['user_id'].unique())
viewing_df_grouped = viewing_df_grouped[viewing_df_grouped['video_playtime_minutes'] > 0]
viewing_df_grouped
gr1 = viewing_df_grouped[viewing_df_grouped['group'] == 'experimental_group']['video_playtime_minutes']
gr2 = viewing_df_grouped[viewing_df_grouped['group'] == 'control_group']['video_playtime_minutes']
stats.ttest_ind(gr1, gr2, alternative='greater')
...
print("control group:") 
plot_graphs(selected_viewing_df[selected_viewing_df['group']=='control_group'])
print("experimental group:") 
plot_graphs(selected_viewing_df[selected_viewing_df['group']=='experimental_group'])
