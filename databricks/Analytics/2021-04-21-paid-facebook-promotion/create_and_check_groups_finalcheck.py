...
tracking
| where videoPlayTime >=0 and isTest == false and startofday(timestamp) >= startofday(datetime('2021-4-21')) and startofday(timestamp) <= startofday(datetime('2021-6-28')) - 1d and channel == '{}'
...
selected_viewing_df = viewing_df_nfnnl.copy()
user_id_list = user_id_nfnnl
...
def process_sub_df(subscription_df, start_date):
  ...
  subscription_df['nr_last_possible_viewing_days'] = subscription_df['nr_last_possible_viewing_days'].apply(lambda x: 1 if x <= 0 else x)
...
def get_grouped_df(viewing_df, current_date = datetime(2021,4,20,0,0,0), past_days = 56):
  ...
  subscription_df_processed = process_sub_df(subscription_df, start_date) 
  ...
  print(viewing_df_grouped)
  viewing_df_grouped['video_playtime_minutes'] = (viewing_df_grouped['video_playtime_minutes'] / viewing_df_grouped['nr_last_possible_viewing_days']) * 7
  viewing_df_grouped['video_playtime_minutes'].fillna(0, inplace=True)
...
current_date = datetime(2021,6,28,0,0,0)
past_days = 68
...
current_date = "2021-8-25"
experiment_start_date = "2021-4-21"  #21-04-2021
subscription_data_query = """
subscriptions
| where startofday(datetime({})) == startofday(Date)
| project customer_id = ['Customer.Id'], subscription_id = ['Subscription.Id'], subscription_created_date = ['Subscription.CreatedDate'], subscription_end_date = ['Subscription.SubscriptionEnd'], subscription_plan_id = ['SubscriptionPlan.Id']
""".format(current_date)
subscription_df = run_data_explorer_query(subscription_data_query, kustoDatabase = 'playground').toPandas()
...
final_df.columns = ['customer_id', 'subscription_created_date','platform','group', 'subscription_id', 'subscription_end_date','subscription_plan_id']
...
# add churn measure based on end_date
end_date = datetime(2021,6,28,0,0,0).date()
final_df['churned'] = final_df['subscription_end_date'].apply(lambda x: x.date() <= end_date)

#Exclude people who churned on the day that the experiment started.
#final_df = final_df[final_df['subscription_end_date'] > datetime(2021,4,21,0,0,0)]
