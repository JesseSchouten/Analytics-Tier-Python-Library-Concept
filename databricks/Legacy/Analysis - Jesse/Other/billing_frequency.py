...
query = """
SELECT billing_frequency, type, created as date
FROM static_tables.billing_frequencies
"""
dfs_billing_frequency_id = spark.sql(query)
del query
dfs_billing_frequency_id.show(10)
...
if dfs_billing_frequency_id.select('date').dtypes[0][1] != 'timestamp':
  Created = udf(to_date, TimestampType())
  dfs_billing_frequency_id = dfs_billing_frequency_id.withColumn("date", Created("date"))
...
if type(last_created_date) == type(None):
  ...
  dfs_billing_frequency_id = dfs_billing_frequency_id.filter(dfs_billing_frequency_id['Date']>last_created_date)
  write = True

if dfs_billing_frequency_id.count() == 0:
  write = False
  sys.exit("TERMINATE - No new data to load.")

# COMMAND ----------

cols = ['date', 'billing_frequency', 'type']
dfs_final = dfs_billing_frequency_id.select(cols)
...
write_to_data_explorer(dfs_final, 'BillingFrequency', resource_name, kustoDatabase, write)
