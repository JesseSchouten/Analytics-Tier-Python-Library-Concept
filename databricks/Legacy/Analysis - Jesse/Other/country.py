...
query = """
SELECT country, name, market, date as date
FROM static_tables.country_market 
"""
dfs_country_market = spark.sql(query)
del query
dfs_country_market.show(10)
...
if dfs_country_market.select('date').dtypes[0][1] != 'timestamp':
  Created = udf(to_date, TimestampType())
  dfs_country_market = dfs_country_market.withColumn("date", Created("date"))
...
if type(last_created_date) == type(None):
  ...
  dfs_country_market = dfs_country_market.filter(dfs_subscription_plan_id['Date']>last_created_date)
  write = True

if dfs_country_market.count() == 0:
  write = False
  sys.exit("TERMINATE - No new data to load.")

# COMMAND ----------

cols = ['date', 'country', 'name','market']
dfs_final = dfs_country_market.select(cols)
...
write_to_data_explorer(dfs_final, 'Country', resource_name, kustoDatabase, write)
