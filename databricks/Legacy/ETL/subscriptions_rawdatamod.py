...
import pytz
import math
#from dutchchannels.data_explorer import run_data_explorer_query, write_to_data_explorer #potentially use in future!
...
FROM static_tables.dc_cc AS t1
GROUP BY t1.`Merchant Reference`;
"""

query_dc_sepa =""" 
SELECT t1.`Merchant Reference` AS merchant_reference
FROM static_tables.dc_sepa AS t1
GROUP BY t1.`Merchant Reference`;
"""
...
dfs = dfs.join(dfs_forex_rates.select(['CCY','RATE']),dfs['SubscriptionCurrency'] == dfs_forex_rates['CCY'],'left')
