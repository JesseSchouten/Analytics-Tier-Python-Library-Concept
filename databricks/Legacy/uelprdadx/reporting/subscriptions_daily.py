...
if type(last_update_datetime) == type(None):
  write = True
  time_selection =  "| where Date < startofday(datetime({}))".format(today_datetime.date())
elif last_update_datetime.date() < today_datetime.date() - timedelta(days=1):
  write = True
  time_selection = "| where Date < startofday(datetime({})) and Date >= startofday(datetime({}) + 1d)".format(today_datetime.date(), last_update_datetime.date())
else:
  write = False
...
subscriptions
{}
| project timestamp = Date, subscription_id = ['Subscription.Id'],subscription_plan_id = ['SubscriptionPlan.Id'], recurring_enabled = ['Subscription.RecurringEnabled'], subscription_end_date = ['Subscription.SubscriptionEnd'], state = ['Subscription.State']
| distinct timestamp, subscription_id, subscription_plan_id, recurring_enabled, subscription_end_date, state
""".format(time_selection)

#viewing_data_query should be retrieved in daily batches!
dfs = run_data_explorer_query(subscription_query, resource_name ='uelprdadx', kustoDatabase = 'playground', df_format = 'dfs')

# COMMAND ----------

cols = ['timestamp','subscription_id','subscription_plan_id','recurring_enabled','subscription_end_date', 'state']
dfs = dfs.select(cols)
...
write_to_data_explorer(dfs, 'subscriptions_daily', resource_name, kustoDatabase, write)
