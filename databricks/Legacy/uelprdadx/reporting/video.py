...
CASE
    WHEN ISNULL(parent_id) THEN 'movie'
    ELSE 'serie'
END AS type,
LAST(age_rating) AS age_rating,
LAST(actors) AS actors
FROM static_tables.nfn_hor_tt_wl_video_details_csv
GROUP BY movie_id,parent_id
"""
...
cols = ['movie_id','parent_id','type','age_rating','actors']
dfs = dfs.select(cols)
...
write_to_data_explorer(dfs, 'video', resource_name, kustoDatabase, write)
