...
country_list_current = run_data_explorer_query(query, resource_name ='uelprdadx', kustoDatabase = 'reporting', df_format = 'dfs').toPandas()['id'].unique().tolist()

country_id_lookup_query = """
Country
| project country_id = CountryId, name = Name, market = Market
"""

dfs= run_data_explorer_query(country_id_lookup_query, resource_name ='uelprdadx', kustoDatabase = 'shared', df_format = 'dfs')

country_list = dfs.toPandas()["country_id"].unique().tolist()

if len(country_list) == len(country_list_current):
  write = False
  dbutils.notebook.exit("Dimension table subscriptions is already complete!")
  
new_country_list = set(country_list) - set(country_list_current)
if len(country_list_current) == 0:
  write = True
else:
  dfs = dfs.filter(dfs['country_id'].isin(new_country_list))

# COMMAND ----------

cols = ['country_id','name','market']
dfs = dfs.select(cols)
...
write_to_data_explorer(dfs, 'country', resource_name, kustoDatabase, write)
