...
query = """
winback_customers
| project customer_id, created_date, unique_key = strcat(customer_id,created_date)
"""

winback_customer_list_current = run_data_explorer_query(query, resource_name ='uelprdadx', kustoDatabase = 'reporting', df_format = 'dfs').toPandas()['unique_key'].unique().tolist()

winback_customers_query = """
SELECT 
axinom_id as customer_id, created_date as created_date, CONCAT(axinom_id,created_date) as unique_key
FROM static_tables.campaign_tracking
"""
dfs = spark.sql(winback_customers_query)

winback_customers_list = dfs.toPandas()["unique_key"].unique().tolist()

if len(winback_customers_list) == len(winback_customer_list_current):
  write = False
  dbutils.notebook.exit("Dimension table winback_customers is already complete!")
  
new_winback_customers = set(winback_customers_list) - set(winback_customer_list_current)
if len(winback_customer_list_current) == 0:
  write = True
else:
  dfs = dfs.filter(dfs['unique_key'].isin(new_winback_customers))


# COMMAND ----------

cols = ['customer_id','created_date']
dfs = dfs.select(cols)
...
write_to_data_explorer(dfs, 'winback_customers', resource_name, kustoDatabase, write)
