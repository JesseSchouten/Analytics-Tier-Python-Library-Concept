...
billing_frequency_list_current = run_data_explorer_query(query, resource_name ='uelprdadx', kustoDatabase = 'reporting', df_format = 'dfs').toPandas()['id'].unique().tolist()

billing_frequency_id_lookup_query = """
BillingFrequency
| project billing_frequency_id = BillingFrequencyId, type = Type
"""

dfs = run_data_explorer_query(billing_frequency_id_lookup_query, resource_name ='uelprdadx', kustoDatabase = 'shared', df_format = 'dfs')

billing_frequency_list = dfs.toPandas()["billing_frequency_id"].unique().tolist()

if len(billing_frequency_list) == len(billing_frequency_list_current):
  write = False
  dbutils.notebook.exit("Dimension table subscriptions is already complete!")
  
new_billing_frequency_list = set(billing_frequency_list) - set(billing_frequency_list_current)
if len(billing_frequency_list_current) == 0:
  write = True
else:
  write = True
  dfs = dfs.filter(dfs['billing_frequency_id'].isin(new_billing_frequency_list))

# COMMAND ----------

cols = ['billing_frequency_id','type']
dfs = dfs.select(cols)
...
write_to_data_explorer(dfs, 'billing_frequency', resource_name, kustoDatabase, write)
