...
subscriptions_list_current = run_data_explorer_query(query, resource_name ='uelprdadx', kustoDatabase = 'reporting', df_format = 'dfs').toPandas()['id'].unique().tolist()

subscription_lookup_query = """
subscriptions
| project subscription_id = ['Subscription.Id'], subscription_plan_id = ['SubscriptionPlan.Id'],subscription_created_date = ['Subscription.CreatedDate'], customer_id = ['Customer.Id'], billing_frequency_id = ['SubscriptionPlan.BillingFrequency']
| summarize billing_frequency_id = max(billing_frequency_id) by subscription_id, subscription_plan_id,subscription_created_date, customer_id
| distinct subscription_id, subscription_plan_id,subscription_created_date, customer_id, billing_frequency_id
"""
dfs = run_data_explorer_query(subscription_lookup_query, resource_name ='uelprdadx', kustoDatabase = 'playground', df_format = 'dfs')

subscriptions_list = dfs.toPandas()["subscription_id"].unique().tolist()
new_subscriptions_list = set(subscriptions_list) - set(subscriptions_list_current)

if len(subscriptions_list_current) == 0:
  write = True
elif len(subscriptions_list) == len(subscriptions_list_current):
  write = False
  dbutils.notebook.exit("Dimension table subscriptions is already complete!")
else:
  write = True
  dfs = dfs.filter(dfs['subscription_id'].isin(new_subscriptions_list))

# COMMAND ----------

cols = ['subscription_id','subscription_plan_id','subscription_created_date','customer_id', 'billing_frequency_id']
dfs = dfs.select(cols)
...
write_to_data_explorer(dfs, 'subscriptions_static', resource_name, kustoDatabase, write)
